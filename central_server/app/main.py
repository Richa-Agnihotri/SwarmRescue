...
from app.event_tracker import EventTracker   # NEW IMPORT
...
async def receive_drone_telemetry(payload: DroneTelemetry, db: Session = Depends(get_db)):
    ...
    # -----------------------------
    tracked_payload = tracker.update(payload.dict())

    # Optional: Print active events in terminal
    tracker.print_active_events()

    print(f"💾 Saved to DB -> Drone {payload.drone_id} committed record ID: {db_log.id}")

    # Broadcast the tracked payload so frontend also gets event_id/event_type
    await manager.broadcast(tracked_payload)

    return {
        "status": "saved_and_ingested",
        "record_id": db_log.id,
        "event_id": tracked_payload["event_id"],
        "event_type": tracked_payload["event_type"]
    }
...
def get_telemetry_history(drone_id: int = None, db: Session = Depends(get_db)):
    ...
    for log in logs:
        formatted_logs.append({
            ...
            "coords": [float(x) for x in log.coords.split(",")],
